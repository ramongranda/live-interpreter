...
- **Translation is a single Ollama HTTP backend.** `src/translate/` = `Translator(HttpTranslator)` over `http.rs` (plus `keep_alive` to pin the model in VRAM); strips `<think>…</think>` (reasoning models) via `strip_think`. First-token latency bench: `benches/translate_latency.rs`.
