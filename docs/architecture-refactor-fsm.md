...
- **R8-tail:** ✅ quitados `tauri`/`tauri-build`/`desktop-native` + `build.rs` (árbol de deps más ligero, build verde). ✅ **WS provider migrado (HECHO, verde):** borrados `StreamEvent`/`StreamStart` de `types.rs`; el WS `/v1/stream/meeting` de `routes.rs` ahora habla `PipelineEvent` vía `EventHub` (fan-out por `tokio::sync::broadcast`, una task forward serializa cada `EventEnvelope` a **bincode** → `Message::Binary`; el `AudioFrame` lleva el PCM s16le inline vía `voice::wav_to_audio_frame`, sin el split `AudioStart`+WAV; lane `Local`, `lang` derivado de `Direction::source/target`). El handshake sigue `SessionStart` (JSON Text) + audio WAV (Binary). Consumidores Rust actualizados: `live-interpreter-client` (usa los tipos del lib, decodifica `EventEnvelope` bincode, reconstruye WAV desde PCM+`AudioSpec`) y `live-interpreter-ws-smoke`. `process_audio_path` se conserva como worker. Encoding bincode (no JSON como `li-interpret`/fsm-ui.html) porque los consumidores del meeting stream son binarios Rust y transporta audio. 97 tests lib verdes, clippy `-D warnings` limpio, release `native-audio` verde. ✅ **`LI_ROLE` (colapsar `live-interpreter-client` en `main.rs`): DESCARTADO.** La reorientación Candle (§7.2: supervisor de tasks Tokio, no de procesos OS) anuló la razón original de FASE-4 (un binario que el supervisor spawnea por rol). El cliente es un proceso de red distinto del server HTTP — colapsarlo mezclaría concerns; y `li-interpret`/`li-mesh` ya cubren la captura nativa local/LAN. `live-interpreter-client` se mantiene como binario aparte para el caso "cliente cpal → server WS remoto". **R8-tail cerrado.**
- **R9:** spike `CandleQwen3Backend` (research; cuando el resto esté sólido).
- **R10 (HECHO, verde):** la voz traducida viaja a otro nodo. El `mesh.rs` (libp2p: mDNS + gossipsub salud/VRAM + request-response audio bincode) ya existía pero con procesadores stub; ahora **cableado al pipeline real**. Nuevo `src/capture.rs` (cpal + VAD energético compartido, deduplicado de `li-interpret`) y `src/mesh_pipeline.rs` (bridge SRP: `pcm_s16le_to_f32`/`f32_to_pcm_s16le`/`collect_interpreted` puros + testeados, y `PipelineMeshProcessor: MeshAudioProcessor` que corre `interpret_utterance` y devuelve `AudioTaskResult` con la voz traducida). Binario `li-mesh` (`--features native-audio`) con `LI_ROLE=provider` (Whisper+Ollama+Qwen3-TTS sirviendo tareas de audio de la malla, telemetría NVML) | `consumer` (cpal+VAD → `MeshCommand::SubmitAudio` al mejor provider por mDNS → la voz traducida vuelta → `PipewireVirtualMic`). **Verificado en vivo:** test de integración `tests/mesh_roundtrip.rs` (dos nodos libp2p reales en loopback, round-trip `AudioChunk`→`AudioTaskResult`, ~2.5–5.5s, `#[ignore]` porque necesita mDNS UDP-multicast) + binario provider arranca, carga Whisper y anuncia GPU por mDNS.
- **R10-clon-cross-node (HECHO, verde):** el timbre del consumer **viaja por el cable**. `AudioChunk` lleva `Option<VoiceReference>` (samples f32 + transcript + consent); el consumer lo adjunta en el primer chunk de sesión (`capture::read_wav_f32` de `LI_VOICE_REF`), el provider lo cachea por `session_id` (`PipelineMeshProcessor::resolve_voice`) y renderiza `MyProfile` (consent-gated vía `profile_from_reference`), así los chunks siguientes (sin ref) reusan el clon. Sin ref → neutro. 74 tests lib verdes, round-trip de integración verde.
- **R10-hardening (HECHO, verde) — evaluación InferMesh/LocalAI.** Contrastado el mesh actual contra `redbco/infermesh` (GPU-aware inference mesh en Rust), LocalAI P2P, Dynamo, Parallax: el core (peer discovery + gpu telemetry + load-aware routing + health + failover) **ya estaba**. Conclusión: *disaggregation* (ASR en nodo X, TTS en nodo Y, à la InferMesh/Dynamo) **se descarta** — suma round-trips y daña la latencia conversacional; el pipeline monolítico-por-nodo es correcto para voz. Model/task-aware routing = YAGNI hasta nodos heterogéneos. De la inspiración se levantaron solo 2 cosas de alto ROI:
  1. **Routing por latencia.** `ProviderScore.avg_latency_ms` (EWMA 3:1, medido en el round-trip de cada tarea); `rank_key` ahora prioriza menor latencia → VRAM → sesiones. Latencia desconocida = `LATENCY_UNKNOWN_RANK_MS` (3s): se prueba sobre un nodo conocido-lento pero no desbanca a uno conocido-rápido. `observe_health` preserva la latencia medida entre refrescos.
  2. **`LI_MESH_TOKEN`** (gap de seguridad; el server HTTP ya tenía `LI_AUTH_TOKEN`, el mesh no). `MeshConfig.auth_token` + `token` en `MeshHealth` + `auth_token` en `AudioChunk`; `token_matches` (puro, `None`=malla abierta) filtra providers con token distinto y el provider descarta tareas no autorizadas (→ el consumer hace failover). 80 tests lib verdes, round-trip verde.
- **R11 — Voice Runtime Diferencial (privacidad + degradación + explainability).** Evaluado un segundo lote de feedback (capability registry, routing por coste, warm pools, ExecutionMode, SessionGraph, plugin `ModelBackend`). La mayoría **ya existe** (`ProviderScore::rank_key` = routing por coste latency→VRAM→sesiones; `LI_ROLE` provider/consumer + `li-interpret` = los 3 ExecutionMode; provider carga Whisper al boot = warm pool informal; `RuntimeHealth`/`ComponentState` = estados de modelo; los traits `VoiceSynthesisBackend`/`Transcriber`/`TextTranslator` = la frontera plugin). **YAGNI hasta nodos heterogéneos:** `loaded_models`/`supported_tasks`, `model_loaded_bonus`/`cold_start`, `ModelBackend` genérico, `SessionGraph` — hoy todo provider carga todo, no discriminan nada. **Dos decisiones de arquitectura tomadas:**
  - **Clon de voz cross-node: se mantiene.** El `VoiceReference` (samples f32, consent-gated) sigue viajando por el cable (R10-clon-cross-node). `VoiceUsagePolicy` lo codifica como política explícita y togglable (`allow_remote_synthesis` default `true` = comportamiento actual), no se revierte.
  - **Disaggregation (ASR en nodo X, TTS en nodo Y): sigue rechazada.** Confirma R10-hardening: la malla enruta la **utterance entera** al mejor nodo, no por-stage. La UI de explainability explica ruteo whole-utterance (`"utterance → RTX4090: Whisper warm, 18ms RTT"`), no split.
  - Subfases (ROI desc): **R11.1 (HECHO, verificado en vivo)** `bench` CLI: `LI_ROLE=bench` con `LI_BENCH_WAV=<wav>` (pipeline completo ASR+translate+TTS) o `LI_BENCH_TEXT="…"` (translate+TTS, sin ASR); mide por etapa (ASR / translate / TTS-first-frame / TTS-full) + **TTFA** (time-to-first-audio = la latencia conversacional real) + total + RTF (vs segundos de audio de entrada) + VRAM (NVML), con un warmup untimed que absorbe el coste de carga. Reusa los componentes reales del provider (`Transcriber`/`Translator`/`HttpQwenBackend`), solo API pública (sin hooks de timing en el core puro). **Medido en vivo (text-mode, neutro):** translate ≈250ms, **TTS ≈2.5s (etapa dominante)**, TTFA mediana ~2.7s — y `tts_first==tts_full` confirma que Qwen3-TTS devuelve **un solo frame** (no streamea sub-cláusula), dato que R7-chunked solapa solo entre cláusulas, no dentro. · **R11.2 (HECHO, verde)** `voice::VoiceUsagePolicy { require_consent, allow_remote_synthesis, allow_profile_export, watermark }` + `from_env` (`LI_VOICE_REQUIRE_CONSENT`/`_ALLOW_REMOTE`/`_ALLOW_EXPORT`/`_WATERMARK`) + gate puro `may_ship_reference(consent)` (testeado). Default = comportamiento shipped (clon cross-node ON, consent requerido). Cableado en el consumer (`li-mesh`): `LI_VOICE_ALLOW_REMOTE=0` → el timbre **no sale del nodo** (provider neutro). · **R11.3 (HECHO, verde)** `quality::QualityTier { Realtime, Balanced, Quality }` + `degrade(tier, RuntimeConditions) → Degradation` puro y testeado (GPU saturada→Realtime/modelo pequeño, clon no disp→`force_neutral`, mesh caída→`force_local`; degradación, no error). Cableado: `LI_QUALITY_TIER` elige el modelo Whisper en provider/bench **solo si el fichero existe** (nunca rompe un arranque que funciona). Verificado en vivo: `tier=Realtime` en el bench. · **R11.4 (HECHO, verde)** `voice::GeneratedAudioMeta { generated_by, voice_profile_id, synthetic, timestamp_ms }` + builder `new(by, route, profile_id, ts)` (clock inyectado → puro; `Neutral` descarta el `profile_id`). Cableado: nuevo campo `Option<GeneratedAudioMeta>` en `AudioTaskResult`, lo estampa el `PipelineMeshProcessor` cuando hay audio (None en text-only/Off). · **R11.5 (HECHO, verde)** `mesh::explain_route(&ProviderScore) → String` puro y testeado (`"utterance → …peer: 18ms RTT, N MB free, K active session(s)"`, **whole-utterance**, no per-stage). Cableado: el consumer lo emite por `tracing::info!` en `best_provider` cada vez que enruta. **90 tests lib verdes** (10 nuevos), round-trip de integración compila.
- **R11.6 (HECHO, verde) — ataque a la latencia TTS (el bottleneck que midió R11.1).** Investigado el servicio Qwen3-TTS con datos, no suposiciones: (1) **no streamea** — `stream:true` → HTTP 400; con `stream:false` el TTFB == total (sintetiza todo y luego devuelve). (2) **Serializa** — 2 requests concurrentes = 8.0s ≈ 2×3.98s (un solo modelo, sin paralelismo). (3) **Latencia ∝ longitud** con ~1s de overhead fijo por llamada ("Hola" 1.2s vs frase de 140 chars ~9–11s). **Conclusión:** el único lever sin cambiar el servicio es **chunking secuencial por cláusula** (baja *time-to-first-audio*, no el total; `render_chunks`/R7 ya lo hace en `li-interpret`). Pero `split_clauses` partía **solo en terminadores** (`.!?;…`) → una frase larga con una sola coma = 1 chunk = sin win. **Mejorado `split_clauses`** (puro, +3 tests): además parte en separadores suaves (`,:—`) cuando el chunk ya es ≥24 chars, y **fuerza corte en límite de palabra** cerca de 72 chars para run-ons sin puntuación; funde el resto diminuto en el chunk previo (el overhead de ~1s no compensa un chunk minúsculo). El bench ahora sintetiza clause-chunked: `tts_first` = primera cláusula = TTFA real. **Medido en vivo:** frase run-on **TTFA 11.2s → 6.7s** (≈mitad, `clauses=2`); total sigue ~11s (synth serializado, esperado). Mejora `li-interpret` (R7) y el bench. **93 tests lib verdes.** El win mayor (chunking sobre la malla) se aborda en R11.7; el fix de fondo del piso TTS sigue siendo un backend más rápido / GPU detrás del trait `VoiceSynthesisBackend` (GPU bloqueada por `nvcc`).
- **R11.7 (HECHO, verificado en vivo) — mesh clause streaming.** Lleva el win de R11.6 al path distribuido: el provider entrega la traducción **cláusula a cláusula** en lugar de un blob, así el consumer reproduce la cláusula 1 mientras el provider sintetiza la 2 (análogo distribuido de R7). **Decisión de transporte:** `libp2p-stream` no está en el umbrella 0.56 (crate aparte) y request-response es 1:1, así que se hizo **request-response bidireccional** reusando el transporte existente (sin dep nueva): el `AudioRequest` es un enum `Submit(AudioChunk)` (consumer→provider) | `Deliver(AudioSegment)` (provider→consumer), y `AudioResponse` = `Accepted`/`DeliverAck`/`Rejected`. Reemplaza `AudioTaskResult` por **`AudioSegment { session_id, sequence, clause_index, last, transcription, translation, tts_*, meta, auth_token }`** (la provenance R11.4 ahora viaja por cláusula). El trait `MeshAudioProcessor::process` pasa a **`process_stream(chunk, mpsc::Sender<AudioSegment>)`**; `PipelineMeshProcessor` hace ASR+translate una vez y sintetiza cada cláusula emitiéndola al terminar. **Concurrencia:** la síntesis corre en una task spawneada (ya no bloquea el run loop) que inyecta cada `Deliver` por un canal interno; el run loop (único dueño del swarm) lo emite. **Orden:** como request-response **no garantiza orden** entre requests, la entrega es **ack-gated** (cláusula N+1 solo tras el `DeliverAck` de N) — descubierto porque el e2e real recibía la cláusula `last` antes que la 0 y descartaba la 0; la síntesis igual corre adelante (buffered), preservando el win. El consumer (`li-mesh`) registra un sink por `(session, sequence)` y reproduce cada cláusula al llegar (timeout 30s/cláusula). Failover de `Submit` preservado; fallo de `Deliver` corta el stream. **Verificado en vivo:** test unitario `clauses_stream_incrementally_not_after_full_synthesis` (mock TTS con gate prueba que la cláusula 0 sale **antes** de sintetizar la 1) + `tests/mesh_roundtrip.rs` reescrito a streaming **corre y pasa con dos nodos libp2p reales + mDNS** (2 cláusulas en orden; `#[ignore]` solo por el multicast en CI). **97 tests lib verdes** (codec roundtrip de `AudioRequest`/`AudioResponse`/`AudioSegment` incluido).
- **R11.8 (HECHO, verificado en vivo) — GPU desbloqueada + validación e2e del pipeline real.** El blocker repetido de todo el roadmap (`nvcc` ausente → sin whisper-GPU ni candle-cuda) **resuelto**: instalado **CUDA Toolkit 12.9** (repo oficial NVIDIA, `cuda-toolkit-12-9`) sobre el driver 590 (CUDA 13.1), que soporta **Blackwell sm_120** de la RTX 5060 Ti; el ggml de `whisper-rs-sys 0.15` ya lista `120a-real` y compila con `arch=compute_120a,code=[sm_120a]` (default `CMAKE_CUDA_ARCHITECTURES=native`). Env en `~/.bashrc` (`CUDA_PATH`/`CUDACXX`/`PATH`/`LD_LIBRARY_PATH`). Build `--features cuda,native-audio`. **Medido en vivo (bench WAV, mismo `reference.wav` que en CPU):** Whisper **turbo en GPU** → **ASR 7048ms → 359ms (~20×)**, precisión arreglada (*"En un lugar de la mancha de cuyo nombre no quiero acordarme… hidalgo"* legible vs el destrozo de `base`), **RTF 1.32 → 0.80** (más rápido que tiempo real), TTFA 11.0s → 3.1s. VRAM: turbo ~1.6GB, cabe con Qwen(10GB)+Ollama(2.3GB). El piso de latencia se mueve de ASR a **TTS** (ahora la etapa dominante). **Decisión TTS (precisión/estabilidad/fiabilidad sobre quick-win): Qwen3-TTS se queda.** Evaluado Kokoro-ONNX (no-AR, ~50× rápido) y **descartado**: añade 2 deps externas frágiles (ONNX runtime `ort` + `espeak-ng` para G2P), la correctitud del fonemizador es un riesgo de calidad, solo acelera la lane **Neutral** (no clona), y rompe el thesis Candle-puro. Qwen es preciso, **clona el timbre** (el diferenciador), ya está en GPU y es estable; su latencia se mitiga con el clause-chunking (R11.6/R11.7), no cambiando de modelo. **Validación e2e del stack completo:** nuevo `tests/mesh_real_pipeline.rs` (`#[ignore]`) — dos nodos libp2p reales donde el provider corre el pipeline **real** (Whisper GPU + Ollama + Qwen3-TTS) y streamea la traducción cláusula-a-cláusula al consumer. **Corre y pasa en vivo (20.7s):** `reference.wav` ES → 3 cláusulas EN en orden con audio Qwen real (86k/146k/88k samples @24kHz). Cierra el loop: GPU ASR preciso → translate → clon TTS → streaming R11.7, todo junto, en el producto real.
